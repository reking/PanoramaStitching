...
vector<Point2f> Normalize(const vector<Point2f> & Key, Mat &T)
{
  ...
  //  cout<<"The xm value is: "<<xm<<endl;
    ...
    for(i = 0; i < Num; i++)
    {
        ...
        //KeyNorm[i].x = scale*Key[i].x + tx;
        //KeyNorm[i].y = scale*Key[i].y + ty;
    }
...
void FeatureExtraction(const Mat& img1, const Mat& img2, vector<Point2f> & Key1, vector<Point2f> & Key2)
{
    ...
    for(int i = 0; i < descriptor1.rows; i++)
    {
        if(matches[i].distance < 5*min_dist && Key1.size() < 2000)
        {
...
struct ReprojectionCostFunctor{
    ...
    //Vec2 y_;
    //public:
        ReprojectionCostFunctor(const Vec2 & x1, const Vec2 & y1) : x_(x1), y_(y1){ }
        //ReprojectionCostFunctor() { }
        template<typename T>
            bool operator()( const T* H_parameters,  const T* x_true_parameters,  const T* y_true_parameters, T* residual) const
            {
                typedef Eigen::Matrix<T, 3, 3> Mat3;
                typedef Eigen::Matrix<T, 2, 1> Vec2;
                typedef Eigen::Matrix<T, 3, 1> Vec3;
                Mat3 H(H_parameters);
                Vec2 x_meature(T(x_(0)),T(x_(1)));
                Vec2 y_meature(T(y_(0)),T(y_(1)));
                Vec2 x_true(x_true_parameters);
                Vec2 y_true(y_true_parameters);
                residual[0] = (x_meature(0) - x_true(0))*(x_meature(0) - x_true(0));
                residual[1] = (x_meature(1) - x_true(1))*(x_meature(1) - x_true(1));
                residual[2] = (y_meature(0) - y_true(0))*(y_meature(0) - y_true(0));
                residual[3] = (y_meature(1) - y_true(1))*(y_meature(1) - y_true(1));
                Vec3 Temp;
                Temp(0) = x_true(0);
                Temp(1) = x_true(1);
                Temp(2) = T(1.0);
                ...
                residual[4] = (Temp(0) - y_true(0))*(Temp(0) - y_true(0))*T(10.0);
                residual[5] = (Temp(1) - y_true(1))*(Temp(1) - y_true(1))*T(10.0);
                return true;
            }
        static ceres::CostFunction* Create(const Vec2 &x1, const Vec2 &x2)
        {
            return (new ceres::AutoDiffCostFunction<ReprojectionCostFunctor,6,9,2,2>(new ReprojectionCostFunctor(x1,x2)));
        }
            Vec2  x_;
            Vec2  y_;
...
struct ReprojectionCost{
    template <typename T = Point2f> bool operator()(const T & x_meature, const T & x_true, double* residual) const
    {
        residual[0] = std::pow(x_meature.x - x_true.x, 2.0);
        residual[1] = std::pow(x_meature.y - x_true.y, 2.0);
        return true;
    }
};

struct LagrangeNorm{
    template <typename T = Point2f> bool operator() (const T & x_true, const T & y_true, const Mat & H, double* residual) const
    {
        Point2f Temp;
        Temp = H*x_true;
        residual[0] = std::pow(Temp.x - y_true.x, 2.0)*10;
        residual[1] = std::pow(Temp.y - y_true.y, 2.0)*10;
    }
}*/
bool ReprojectionOptimization(Mat3 &H, const vector<Vec2> &Key3, const vector<Vec2> &Key4)
{
    int num = 0;
    if(Key3.size() == Key4.size())
         num = Key4.size();

    vector <Vec2> Var1(Key3);
    vector <Vec2> Var2;
    Vec3 Temp;
    Vec2 temp;
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        Temp(2) = 1.0;
        Temp = H*Temp;
        temp(0) = static_cast<double> (Temp(0) / Temp(2));
        temp(1) = static_cast<double> (Temp(1) / Temp(2));

        Var2.push_back(temp);
    }
    cout<<"ceres is ready now!"<<endl;

        ceres::Problem problem;
        for(int i = 0; i < num; i++)
        {
            ceres::CostFunction* cost_function =
                ReprojectionCostFunctor::Create(Key3[i], Key4[i]);

            problem.AddResidualBlock(cost_function,
                    NULL,
                   H.data(), Var1[i].data(), Var2[i].data());
        }
        //Solve The Problem
        ...
        //Terminate if the average distance is good enough
...
int main(int argc, char *argv[])
{
    ...
    //Transfer from opencv format to eigen format
    vector<Vec2> Key5(Key1.size());
    vector<Vec2> Key6(Key2.size());
    Mat3 H_eigen;

    cout<<"transfer to eigen format ok!"<<endl;
    ...
    for(int i = 0; i <9; i++)
    {
        H_eigen(i) = H_final.at<double>(i/3,i%3);
    }
    
    
    
    if(ReprojectionOptimization(H_eigen,Key5,Key6))
    {
       cout<<"Optimization is OK!"<<endl;
        for(int i = 0; i < 9; i++)
        {
            cout<<"The H value is "<<H_eigen(i)<<endl;
            H_final.at<double>(i/3,i%3) = H_eigen(i);
    ...
    Key5.resize(Key3.size());
    Key6.resize(Key3.size());
    ...
    for(int i = 0; i <9; i++)
    {
        H_eigen(i) = H_final2.at<double>(i/3,i%3);
    }
    ...
    if(ReprojectionOptimization(H_eigen,Key5,Key6))
    {
        ...
        {
            cout<<"The H value is "<<H_eigen(i)<<endl;
            H_final2.at<double>(i/3,i%3) = H_eigen(i);
    ...
    Key5.resize(Key4.size());
    Key6.resize(Key4.size());
    ...
    for(int i = 0; i <9; i++)
    {
        H_eigen(i) = H_final3.at<double>(i/3,i%3);
    }

    
    cout<<"The Key5 size is"<<Key5.size()<<endl; 
    if(ReprojectionOptimization(H_eigen,Key5,Key6))
    {
        cout<<"Optimization is OK!"<<endl;
        for(int i = 0; i < 9; i++)
        {
            cout<<"The H value is"<<H_final3.at<double>(i/3,i%3)<<endl;
            cout<<"The H_eigen value is "<<H_eigen(i)<<endl;
            H_final3.at<double>(i/3,i%3) = H_eigen(i);
